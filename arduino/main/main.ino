...
bool init_PIDs()
{
    ...
    for (int pid_id = 0; pid_id < MAX_PID; pid_id++)
    {
        if (pids[pid_id] <= 0x20)
        {
...
{
    for (int pid_id = 0; pid_id < MAX_PID; pid_id++){
        if (pids_supp_mask[pid_id] == 1 || true){ // if pid supported does work please remove the true sentence here
            int __value = 0;
            int ret = getPid(pid_id, &__value);
            if(ret)
            {
                Serial.print(pid_id);
                Serial.print(",");
